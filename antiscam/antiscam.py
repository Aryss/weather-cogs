...
class antiscam:
    ...
    __version__ = "ASL-v1.0"

    def __init__(self, bot):
        self.bot = bot
        self.location = 'data/antiscam/settings.json'
        self.json = dataIO.load_json(self.location)
        self.regex = re.compile(r"<?(https?:\/\/)?(www\.)?(fortnite-special\.com\/)\b([-a-zA-Z0-9/]*)>?")
#        self.regex_discordme = re.compile(r"<?(https?:\/\/)?(www\.|gaming\.)?(youtube\.com\/|youtu\.be\/)\b([-a-zA-Z0-9/]*)>?")
    ...
    async def antiscamset(self, ctx):
        """Manages the settings for antiscam."""
    ...
    @antiscamset.command(pass_context=True, no_pm=True)
    @checks.admin_or_permissions(administrator=True)
    async def toggle(self, ctx):
        """Enable/disables antiscam in the server"""
        serverid = ctx.message.server.id
        if self.json[serverid]['toggle'] is True:
            self.json[serverid]['toggle'] = False
            await self.bot.say('antiscam is now disabled')
        elif self.json[serverid]['toggle'] is False:
            self.json[serverid]['toggle'] = True
            await self.bot.say('antiscam is now enabled')
    ...
    async def toggledm(self, ctx):
        ...
        if self.json[serverid]['dm'] is False:
            ...
            await self.bot.say('Enabled DMs on removal of stream/video links')
    ...
    @antiscamset.group(pass_context=True, no_pm=True)
    @checks.admin_or_permissions(administrator=True)
    async def exclude(self, ctx):
        """Exclude the channels where antiscam will be active"""
        if ctx.invoked_subcommand is None:
            await send_cmd_help(ctx)
...
def check_folder():
    if not os.path.exists('data/antiscam'):
        os.makedirs('data/antiscam')


def check_file():
    f = 'data/antiscam/settings.json'
