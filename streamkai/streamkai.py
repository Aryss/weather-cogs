...
class streamkai:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.twitch_streams = dataIO.load_json("dama/streamkai/twitch.json")
        self.hitbox_streams = dataIO.load_json("dama/streamkai/hitbox.json")
        self.mixer_streams = dataIO.load_json("dama/streamkai/beam.json")
        self.picarto_streams = dataIO.load_json("dama/streamkai/picarto.json")
        settings = dataIO.load_json("dama/streamkai/settings.json")
    ...
    @streamkaiset.command(pass_context=True, no_pm=True)
    @checks.admin()
    ...
    async def twitch_online(self, stream):
        ...
        if r.status == 200:
            if data["stream"] is None:
                raise OfflineStream()
            elif channel["game"] == self.settings["GAME"]:
...
def check_folders():
    if not os.path.exists("dama/streamkai"):
        print("Creating dama/streamkai folder...")
        os.makedirs("dama/streamkai")
...
def check_files():
    ...
    for filename in stream_files:
        if not dataIO.is_valid_json("dama/streamkai/" + filename):
            print("Creating empty {}...".format(filename))
            dataIO.save_json("dama/streamkai/" + filename, [])

    f = "dama/streamkai/settings.json"
