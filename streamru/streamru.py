...
class Streamru:
    ...
    async def twitch(self, ctx, stream: str):
        ...
        try:
            ...
            await self.bot.say(stream + " сейчас оффлайн.")
        except StreamNotFound:
            await self.bot.say("Такой канал не существует.")
        except APIError:
            await self.bot.say("Ошибка связи с API.")
        except InvalidCredentials:
            await self.bot.say("Для владельца: Client-ID или неверен или не установлен. "
                               "см `{}streamset twitchtoken`"
                               "".format(ctx.prefix))
        else:
            await self.bot.say(embed=embed)
    ...
    def twitch_embed(self, data):
        ...
        embed.add_field(name="Фолловеров", value=channel["followers"])
        embed.add_field(name="Всего просмотров", value=channel["views"])
        embed.set_thumbnail(url=logo)
        if data["stream"]["preview"]["medium"]:
            embed.set_image(url=data["stream"]["preview"]["medium"] + self.rnd_attr())
        if channel["game"]:
            embed.set_footer(text="Играет в " + channel["game"])
                ...
                for stream in streams_list:
                    ...
                    try:
                        ...
                        for channel_id in stream["CHANNELS"]:
                            ...
                            message = mention + " {} стримит!".format(stream["NAME"])
